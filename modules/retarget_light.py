...
class Retargetor():
    ...
    def __call__(self, 
        ...
        for idx, target in enumerate(sim_cfg):
            ...
            for light in self.scene.get_all_lights():
            ...
            for _ in range(2):
                ...
                self.scene.add_directional_light(direction, [color[0]*intensity, color[1]*intensity, color[2]*intensity])
            ...
            # self.scene.add_directional_light(dir.tolist(), [2.8, 2.8, 2.8])
...
if __name__ == '__main__':
    retargetor = Retargetor(
        ur5_hand_urdf_path="ur5_leaphand/ur5_leaphand.urdf",
        ur5_gripper_urdf_path="ur5_leaphand/ur5_gripper.urdf",
        device='cuda:0'
    )
    
    import json
    with open('sim_cfg.json', 'r') as f:
        sim_cfg = json.load(f)
    
    img = cv2.imread('inpainted_img.jpg')
    img = retargetor(img, sim_cfg, 0)
    cv2.imwrite('retargeted_img.jpg', img)